...
One Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- ThreadRPCServer : Remote procedure call handler, listens on port 9876 for connections and services them.
...
However, in open source software such as One Core, where everyone uses
...
pay attention to for reviewers of One Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: One Core uses tinyformat, which is type safe. Leave them out to avoid confusion
