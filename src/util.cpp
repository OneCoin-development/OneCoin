...
const char * const BITCOIN_CONF_FILENAME = "one.conf";
const char * const BITCOIN_PID_FILENAME = "oned.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "one" is a composite category enabling all One-related debug output
            if(ptrCategory->count(string("one"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\OneCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\OneCore
    // Mac: ~/Library/Application Support/OneCore
    // Unix: ~/.onecore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "OneCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/OneCore";
#else
    // Unix
    return pathRet / ".onecore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty one.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override one.conf
