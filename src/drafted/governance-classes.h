 ...
 *
 *   // network
 *   COneNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.one.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class COneNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
// // can be under: OneNetwork
// //   -- signature requirements : Key1(User)
// class COneNetworkVariable : public CGovernanceObject
// {
// private:
